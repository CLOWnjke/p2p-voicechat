...
//! Поэтому закрытие окна прячет его в значок, а по-настоящему выйти можно
//! из меню значка.
//!
...
mod imp {
    ...
    impl Tray {
        pub fn new(rgba: Vec<u8>, w: u32, h: u32) -> Option<Self> {
            ...
            let quit = MenuItem::with_id("quit", "Выйти из комнаты", true, None);
            menu.append(&open).ok()?;
            menu.append(&mute).ok()?;
            ...
            let icon = TrayIconBuilder::new()
                ...
                .with_tooltip("voicechat — комната открыта")
                .with_icon(icon)
                .build()
                .ok()?;
